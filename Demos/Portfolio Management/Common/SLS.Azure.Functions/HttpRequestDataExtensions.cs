...
/// <summary>
/// Extensions to the <see cref="HttpRequestData"/> type.
/// </summary>
public static class HttpRequestDataExtensions
{

	/// <summary>
	/// Gets a request object with paging information from the provided <see cref="HttpRequestData"/>.
	/// </summary>
	/// <typeparam name="T">The type of the request object to be returned.</typeparam>
	/// <param name="httpRequestData">The <see cref="HttpResponseData"/> to be interrogated for the request object.</param>
	/// <param name="routeValues">Any route values supplied to the Azure Function.</param>
	/// <param name="jsonSerializerOptions">The serializer objects for formatting the request body.</param>
	/// <returns>A <typeparamref name="T"/> representing the request object from the HTTP request.</returns>
	public static async Task<T> GetRequestWithPagingParametersAsync<T>(this HttpRequestData httpRequestData, Dictionary<string, string> routeValues, JsonSerializerOptions jsonSerializerOptions) where T : IPagedRequest, new()
	{
		T requestObject = await httpRequestData.GetRequestParametersAsync<T>(routeValues, jsonSerializerOptions);
		requestObject.PageSize ??= 0;
		requestObject.PageIndex ??= 0;
	...
	public static string GetContentLocation(this string route)
	{
		...
		try
		{
			string httpsStatus = Environment.GetEnvironmentVariable("HTTPS");
			if (!string.IsNullOrWhiteSpace(httpsStatus) && httpsStatus.ToLower() == "on")
				httpProtocol = "https";
			else
				httpProtocol = "http";
		}
	...
	public static bool CreatIfNotExists(this HttpRequestData httpRequestData)
	{
		try
		{
	...
	/// <summary>
	/// Creates a HTTP Status 201 response object.
	...
	/// <summary>
	/// Attempts to get the value of a specific request query string element.
	/// </summary>
	/// <param name="httpRequestData">The <see cref="HttpRequestData"/> to use for building the response object.</param>
	...
	public static bool TryGetQueryStringValue(this HttpRequestData httpRequestData, string key, out string value)
	{
		...
		if (TryGetQueryStringValues(httpRequestData, out Dictionary<string, string> values))
	...
	/// <summary>
	/// Attempts to get the values from the request's query string.
	/// </summary>
	/// <param name="httpRequestData">The <see cref="HttpRequestData"/> to use for building the response object.</param>
	/// <param name="queryStringValues">The resulting dictionary of the query string values.</param>
		...
		if (!string.IsNullOrWhiteSpace(httpRequestData.Url.Query))
		{
			...
			if (queryValues.Count >= 1)
			{
				...
				foreach (string key in queryValues.Keys)
					result.TryAdd(key.ToLowerInvariant(), queryValues[key]);
				return result;
			}
			else
				return default;

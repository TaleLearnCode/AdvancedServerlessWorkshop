...
public abstract class CacheResponseBase2
{

	private readonly Container _cosmosContainer;

	protected CacheResponseBase2(Container container)
	{
	...
	protected async Task<Community?> GetCommunityAsync(PortfolioContext portfolioContext, string communityNumber)
	{
	...
	protected async Task<int> GetCommunityStartingAtPriceAsync(PortfolioContext portfolioContext, int communityId)
	{
	...
	protected async Task<Dictionary<string, PricingByCareTypeResponse>?> GetCommunityPricingAsync(
		PortfolioContext portfolioContext,
		int communityId,
		...
		string? defaultLanguageCulture)
	{
		...
		if (communityCommunityAttributes.Any())
		{
			...
			foreach (CommunityCommunityAttribute communityCommunityAttribute in communityCommunityAttributes)
			{
				...
				string? label = await GetContentCopyAsync(portfolioContext, communityCommunityAttribute.CommunityAttribute.LabelId, languageCulture, defaultLanguageCulture);
				if (label is not null)
				{
					response[attributeTypeKey].Add(new()
					{
						...
						AltText = await GetContentCopyAsync(portfolioContext, communityCommunityAttribute.CommunityAttribute.Icon.AltTextId, languageCulture, defaultLanguageCulture)
					});
				}
			}
			return response;
	...
	protected async Task<string?> GetCommunityPhoneNumberAsync(PortfolioContext portfolioContext, int communityId)
	{
	...
	protected async Task<PostalAddressResponse?> GetCommunityPostalAddressAsync(PortfolioContext portfolioContext, int communityId)
	{
